...
## 5. 서버와 클라이언트의 이해
자 이제 개발의 문턱까지 왔다고 볼 수 있다. 비전공자 분들이 여기까지 읽어 오셨다면 참으로 감사하다...

클라이언트는 웹서비스를 사용하는 사용자이고 서버는 사용자가 어떠한 서비스를 요청할 때 프로그래밍된 로직에 따라서 결과값을 클라이언트에게 응답해 주는 것이다.

국민학식go를 예로 들면 사용자가 식당의 버튼을 클릭하면 요청정보가 서버로 날라가고 서버에서는 버튼의 값에 매칭되는 식단을 클라이언트에게 응답하여 준다.

![](/img/py7.gif "이미지제목")

여기서 서버가 중요하다는 것은 누가봐도 알것이다. 우리는 이 서버 개발을 python언어로 할 것이고 개발할 때 django프레임웤이 우리를 도와줄 것이다.

## 6. 카카오톡 api
여기서 부터는 전공자 혹은 개발에 좀더 관심이 있는 분들이 보면 좋을 것 같다.

카카오톡 옐로아이디 자동응답 api는 카카오톡에서 국민학식go라는 계정을 만들어주고 서버에 요청하는 값과 서버가 응답해야 하는 값들의 틀을 개발해서 제공해주는 서비스이다.
그렇기에 우리는 이 틀에 우리가 원하는 정보만 입력해서 개발하면 된다.

아래는 카카오톡 옐로아이디 api 깃허브 주소이다.
<https://github.com/plusfriend/auto_reply>

## 7. 개발
드디어 개발이다. django프레임워크와 카카오톡api가 워낙 잘 되어 있어서 django를 조금만 공부해보면 누구나 할 수 있는 내용이다. 하지만 django를 공부해야 한다는 조건이 있다.(하하)
